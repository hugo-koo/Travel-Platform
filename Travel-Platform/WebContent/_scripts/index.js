    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.notes.length; i++) {
                ...
                var noteHeader = '' + data.notes[i].noteDtl.notePostDate;
...
/**生成热门游记列表*/
var listHotestNotes = function (page = 1) {
    $("#hotlist").empty();
    createPage(page, "listHotestNotes");
    $('html').animate({ scrollTop: '0px' }, 300);
}
/**生成最新游记列表*/
var listLatestNotes = function (page = 1) {
    $("#latest").empty();
    createPage(page, "listLatestNotes");
    $('html').animate({ scrollTop: '0px' }, 300);
}
/** 生成页面导航键 */
var pageNav = function (page = 1, total = 1, method = "listHotestNotes") {
    var items = "";
    var prev = "";
    var next = "";
    for (var i = 0; i < total; i++) {
        var active = "";
        if (page == i + 1) active = "active";
        items += '<li class="page-item ' + active + '"><a class="page-link" href="javascript: ' + method + '(' + (i + 1) + ')">' + (i + 1) + '</a></li>';
    }
    if (page == 1) {
