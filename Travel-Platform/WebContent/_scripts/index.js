...
var noteCard = (
    ...
    var locationIcon = '<svg style="vertical-align: baseline;" t="1591709547788" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6597" xmlns:xlink="http://www.w3.org/1999/xlink" width="16.9" height="16"><defs><style type="text/css"></style></defs><path d="M511.932268 0c-213.943865 0-387.880498 170.933833-387.880499 381.06211 0 164.815346 238.869364 485.098975 341.66447 620.247558a58.249807 58.249807 0 0 0 46.216029 22.690332c18.129688 0 35.491743-8.443964 46.328916-22.690332 102.704796-135.126006 341.709624-455.432213 341.709624-620.247558C899.970808 170.933833 725.89871 0 511.932268 0z m0 519.574733c-91.393496 0-165.786176-72.902569-165.786176-162.670489 0-89.722765 74.39268-162.738221 165.786176-162.73822 91.438651 0 165.718443 73.015456 165.718443 162.73822 0 89.76792-74.279793 162.670488-165.718443 162.670489z" fill="#0f4c81" p-id="6598"></path></svg>';
    if (noteHeader == 'null') {
        noteHeader = "未命名游记";
    }
    if (toppic == '') {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            for (var i = 0; i < data.notes.length; i++) {
                ...
                var summary = '';
//                var summary = '' + (data.notes[i].noteDtl.noteContent).substring(0, 30) + '...';
                var toppic = '' + data.notes[i].noteDtl.noteToppic;
                if(data.notes[i].regions[0]!=undefined)
                    var location = data.notes[i].regions[0].name;
                else var location = '全球';
            ...
            if (method == "listHotestNotes") {
                ...
                $("#note-link-2").html('<a href="'+$("#hotlist").find("a")[1].href+'">'+$("#hotlist").find("a")[1].text+'</a>');
                $("#note-toppic-3").eq(0).attr("src", $("#hotlist").find("img")[2].src);
                $("#note-link-3").html('<a href="'+$("#hotlist").find("a")[2].href+'">'+$("#hotlist").find("a")[2].text+'</a>');
            }
        },
        error: () => {
            alert("游记获取失败");
